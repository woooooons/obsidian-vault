...
바이너리 데이터(키, 해시, 토큰 등)는 저장·전송을 위해 문자열 인코딩으로 표현됨.  
인코딩 유형은 **문자 집합, 길이, 패딩 여부**로 구분한다.

### 14.1 Hexadecimal (Base16)

- **문자 집합**: `0–9`, `a–f` (또는 `A–F`)
...
- **예시**:
    ...
    `f07469e69ab7eaaf3421a3e616cfaa3415fcd1265577209e0dec5558efe68c87`
...
- **예시**:
    ...
    `8HRp5pq36q80IaPmFs+qNBX80SZVdyCeDexVWO/mjIc=`
...
|유형|특징|비고|
|---|---|---|
|ASCII|7비트 문자|제어문자 포함|
|UTF-8|가변 길이|텍스트용|
|Binary|원시 바이트|사람이 읽기 어려움|

### 14.5 판별 요약

|조건|추정 인코딩|
|---|---|
|`0-9a-f` + 64자|Hex (256비트)|
|`+/=` 포함|Base64|
|`-_` 포함, `=` 없음|Base64URL|
|길이 짝수|Hex 가능성 높음|
|길이 4의 배수|Base64 계열|
